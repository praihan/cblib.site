  ...
  function doStuff() {
    ...
    $(function() {
      ...
      window.CB.Library.parseRSS(url, function(data) {
        for (var key = 0; key < data.length; ++key) {
          var value = data[key];
          var $event = $("#news-" + (key + 1));
          if ($event.length) {
            var eventHref = window.CB.Library.REL_PATH + "/news/#recent-" + (key + 1);
            ...
            $event
              ...
              .append($("<p/>", {"class": "evt-content"}).html(S(value["content:encoded"]["#"]).stripTags().s.cbTruncate(120)))
              .append($("<p/>")
    ...
    $(function() {
      ...
      ee.code = function() {
        ...
        $("body").append($("<div/>", {
          "style": "pointer-events:none;display:block;position:fixed;z-index:999;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0)url('assets/img/misc/krf.png')50%90%no-repeat;"
        }));
        ee.code = function(){};
      }
      ee.load();
